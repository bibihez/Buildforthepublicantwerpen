...
export async function GET(_request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (!source?.file_path) return new Response('Bron heeft geen bestand', { status: 404 });
  const root = path.join(process.cwd(), 'data', 'files');
  const abs = path.resolve(process.cwd(), source.file_path);
  if (!abs.startsWith(root + path.sep) || !fs.existsSync(abs)) return new Response('Bestand niet gevonden', { status: 404 });
